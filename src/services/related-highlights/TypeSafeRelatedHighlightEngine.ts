...
interface ScoreAnswer { type: 'score'; probabilities: Record<string, number> }
interface ChoiceAnswer { type: 'choice'; choice: string }
...
export class TypeSafeRelatedHighlightEngine {
    ...
    private async rerankBatch(profile: RelatedDocumentProfile, batch: RelatedHighlightResult[]): Promise<RelatedHighlightResult[]> {
        ...
        return batch.map((item, index) => {
            ...
            if (relevance?.type !== 'score' || relation?.type !== 'choice') throw new Error('TypeSafe returned an incomplete related-highlight response.');
            const probabilities = relevance.probabilities || {};
            const ai = ((probabilities['1'] || 0) * 0.33 + (probabilities['2'] || 0) * 0.72 + (probabilities['3'] || 0));
            ...
            return { ...item, relevanceScore: ai, finalScore: (item.localScore * 0.25 + ai * 0.75) * relationPenalty,
